...
`show:` whose following indented lines are `name = value` entries.
...
| `start` | identifier | First passage. Default: the first passage in the file. |
...
- Attributes ride after the target separated by `|`: `once`, `time=N`
  ...
  `time`). Invalid combos/values: `TW015` (error).
...
~ unset lamp
~ take "rusty key"
~ take "coin", 3        %% count defaults to 1
~ drop "rusty key"
~ push apples "bruised"
```
...
| Call | Returns | Notes |
| --- | --- | --- |
| `visited(name)` | number | Times the player entered passage `name`. |
| `seen(name)` | boolean | `visited(name) > 0`. |
...
Each choice gets a stable identity: `sha1(passageName + "#" + ordinal)` where
...
(integer), else derived from crypto randomness at first launch and stored in
...
| TW018 | note | Person/POV drift (style lint; heuristic, off under `--no-style`). |
...
Export paths are validated to stay inside the working directory when
